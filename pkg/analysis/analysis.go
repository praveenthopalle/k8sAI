...
import (
	...
	"github.com/k8sAI-ai/k8sAI/pkg/analyzer"
	"github.com/k8sAI-ai/k8sAI/pkg/cache"
	"github.com/k8sAI-ai/k8sAI/pkg/common"
	"github.com/k8sAI-ai/k8sAI/pkg/custom"
...
func NewAnalysis(
	...
	if len(configAI.Providers) == 0 {
		return nil, errors.New("AI provider not specified in configuration. Please run k8sAI auth")
	}
	...
	if aiProvider.Name == "" {
		return nil, fmt.Errorf("AI provider %s not specified in configuration. Please run k8sAI auth", backend)
	}
	...
	if len(a.Filters) != 0 {
		for _, filter := range a.Filters {
			if analyzer, ok := analyzerMap[filter]; ok {
				...
				a.Errors = append(a.Errors, fmt.Sprintf("\"%s\" filter does not exist. Please run k8sAI filters list.", filter))
			}
		}
		wg.Wait()
		return
