...
import (
	...
	k8sAIserver "github.com/k8sAI-ai/k8sAI/pkg/server"
...
var ServeCmd = &cobra.Command{
	Use:   "serve",
	Short: "Runs k8sAI as a server",
	Long:  `Runs k8sAI as a server to allow for easy integration with other applications.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(configAI.Providers) == 0 {
			...
			temperature := func() float32 {
				env := os.Getenv("k8sAI_TEMPERATURE")
				if env == "" {
			...
			maxTokens := func() int {
				env := os.Getenv("k8sAI_MAX_TOKENS")
				if env == "" {
			...
			backend = os.Getenv("k8sAI_BACKEND")
			password := os.Getenv("k8sAI_PASSWORD")
			model := os.Getenv("k8sAI_MODEL")
			baseURL := os.Getenv("k8sAI_BASEURL")
			engine := os.Getenv("k8sAI_ENGINE")
			proxyEndpoint := os.Getenv("k8sAI_PROXY_ENDPOINT")
			providerId := os.Getenv("k8sAI_PROVIDER_ID")
			...
			if envIsSet {
				...
				color.Red("Error: AI provider not specified in configuration. Please run k8sAI auth")
				os.Exit(1)
			}
		}
		if aiProvider == nil {
		...
		if aiProvider == nil || aiProvider.Name == "" {
			color.Red("Error: AI provider %s not specified in configuration. Please run k8sAI auth", backend)
			os.Exit(1)
		}
