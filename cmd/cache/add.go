...
var addCmd = &cobra.Command{
	...
	- Azure Blob storage (e.g., k8sAI cache add azure)
	- Google Cloud storage (e.g., k8sAI cache add gcs)
	- S3 (e.g., k8sAI cache add s3)
	- Interplex (e.g., k8sAI cache add interplex)`,
	Run: func(cmd *cobra.Command, args []string) {
		if len(args) == 0 {
			color.Red("Error: Please provide a value for cache types. Run k8sAI cache add --help")
			os.Exit(1)
		}
